...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #self.copy_p = nn.Linear(1024, 1)
        self.copy_p = nn.Linear(config.n_embd, 1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Set attention mask to be anything after end
        # For each one find index of last 2

        attention_mask = (input_ids != 50256).long() | torch.cat([ (input_ids[:, 1:] != 50256), (input_ids[:, -1:] != 50256) ], dim=1).long()

        # allow attention to one eos token
        #attention_mask = torch.cat([attention_mask[:,:2], attention_mask[:,:-2]], dim=-1)
        ...
        new_inputs = input_ids.unsqueeze(-1).repeat(1,1,input_ids.size(-1)).permute(0,2,1)
        avg_attention = transformer_outputs[3][1].mean(dim=1)
        copy_probs = torch.zeros(lm_logits.size()).cuda().scatter_add(2, new_inputs, avg_attention)

        # Combine lm_logits and copy_probs
        copy_prob = torch.sigmoid(self.copy_p(hidden_states))
        #lm_probs = torch.log(copy_prob*(1e-10 + copy_probs) + (1-copy_prob)*torch.softmax(lm_logits, -1))
        lm_probs = torch.log(copy_prob*(1e-10 + copy_probs) + (1-copy_prob)*torch.softmax(lm_logits, -1))
        #lm_probs = torch.log(torch.softmax(lm_logits, -1) + 1e-10)

        loss = None
        if labels is not None:
            # Shift so that tokens < n predict n
            shift_logits = lm_probs[..., :-1, :].contiguous()
        ...
        if not return_dict:
            output = (lm_probs,) + transformer_outputs[1:]
